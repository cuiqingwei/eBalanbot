...
public class JoystickFragment extends SherlockFragment implements JoystickView.OnJoystickChangeListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        eBalanbotActivity.joystickReleased = true;
    ...
    private void newData(double xValue, double yValue, boolean joystickReleased) {
        ...
        eBalanbotActivity.joystickReleased = joystickReleased;
        ...
        mRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (eBalanbotActivity.mChatService == null)
                    return;
                if (eBalanbotActivity.mChatService.getState() == BluetoothChatService.STATE_CONNECTED && eBalanbotActivity.checkTab(ViewPagerAdapter.JOYSTICK_FRAGMENT)) {
                    if (!getResources().getBoolean(R.bool.isTablet) || !eBalanbotActivity.buttonState) { // Don't send stop if the button in the IMU fragment is pressed
                        if (joystickReleased || (xValue == 0 && yValue == 0))
                            eBalanbotActivity.mChatService.write(eBalanbotActivity.sendStop);
                        else {
                            String message = eBalanbotActivity.sendJoystickValues + d.format(xValue) + ',' + d.format(yValue) + ";";
                            eBalanbotActivity.mChatService.write(message);
                        }
                    }
                }
            }
